...
//	if ((self.click == 1) || (self.spawned_extra == 66)) //necromancy
	if (self.click == 1)
	{
		...
		//total = 0.00000;
//		if ((total > 0) && (self.spawned_extra != 66))  //necromancy
		if (total > 0)
		{
				...
				*/
				...
				if ((step < 48) && (total < self.count) && (pos != world.origin))
				{
					...
					if (newmis.classname == "monster_imp_ice") {
						...
						newmis.think = monster_medusa_green;
					} else {
						remove(newmis);
					}
				}
...
//			}
	...
	thinktime self : nextaction;
	self.think = spawn_refresh;
};

void (entity subject, entity bestiary)spawnify = {
	subject.spawned_extra = 1;  //don't respawn spawned spawners!
	if (subject.solid != SOLID_BSP)
	{
		newmis = spawn();
		newmis.spawned_extra = subject.spawned_extra;
...
entity ()scan_monsters = {
	...
	while ( (search != world) ) {
		if ( (search.flags & FL_MONSTER) && (search.spawned_extra == 0)) {
...
void ()multiply_monsters = {
	...
	if (search == world)
	{
		...
		total = 0.00000;
		while ( (search != world) ) {
			//if ( (search.flags & FL_MONSTER) && (search.spawned_extra == 0) && (!(search.spawnflags & ONWALL) && (vlen(search.origin - self.origin) < 100)) ) {
			if ( (search.flags & FL_MONSTER) && (search.spawned_extra == 0) && (!(search.spawnflags & ONWALL_ALIAS)) ) {
				total += 1.00000;
