...
//		acidblob.scale=random(0.5,1);
	...
	else
	{
		...
		org=self.origin+self.proj_ofs+v_forward*8;		//ws: centered projectile to match crosshair
		setsize(acidblob,'0 0 0', '0 0 0');
	}
...
//	newmis.speed=800;//Hardcoded so less info is sent over network
	...
	setorigin(newmis,self.origin+self.proj_ofs+v_forward*8 + v_right*(offset*3) + '0 0 2');		//ws: centered projectile to match crosshair
