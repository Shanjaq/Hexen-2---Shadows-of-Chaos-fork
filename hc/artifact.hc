...
void artifact_touch()
{
	...
	{
		if ((other.cnt_torch + 1)  > MAX_INV)
			return;	
		else
			other.cnt_torch += 1;		
	}
	else if(self.netname == STR_HEALTHBOOST)   // 5 limit
	{
		//if ((other.cnt_h_boost + 1)  > 30||(other.playerclass!=CLASS_CRUSADER&&other.cnt_h_boost + 1  > 15))
		if ((other.cnt_h_boost + 1) > MAX_FLASK)
			return;	
		else
			other.cnt_h_boost += 1; 
		if (other.predebt == 1) {
	...
	else if(self.netname == STR_SUPERHEALTHBOOST) // 1 limit
	{
		if (deathmatch&&(other.cnt_sh_boost + 1) > 2)
			return;	
		else if ((other.cnt_sh_boost + 1) > MAX_URN)
			return;	
		else
	...
	{
		if ((other.cnt_mana_boost + 1) > MAX_KRATER)
			return;	
		else
	...
	{
		if ((other.cnt_teleport + 1) > MAX_INV)
			return;	
		else
	...
	{
		if ((other.cnt_summon + 1) > MAX_SUMMON)
			return;	
		else
			other.cnt_summon += 1;
	...
	{
		if ((other.cnt_invisibility + 1) > MAX_GHOST)
			return;	
		else
	...
	{
		if ((other.cnt_polymorph + 1)  > MAX_POLY)
			return;	
		else
	...
	{
		if ((other.cnt_cubeofforce + 1)  > MAX_CUBE)
			return;	
		else
	...
	{
		if ((other.cnt_invincibility + 1)  > MAX_ICON)
			return;	
		else
...
void use_healthboost()
{
	...
	self.cnt_h_boost -= 1;
	self.health += 25;
	sound (self,CHAN_VOICE,"fx/health.wav",1,ATTN_NORM);
  	if(self.health > self.max_health)
	{
