...
void archer_duck () [++ $duck1..$duck14]
{
	...
	if(cycle_wrapped)
	{
		...
		if (self.classname == "monster_archer")
			...
			//sound (self, CHAN_VOICE, "archer/pain2.wav", 1, ATTN_NORM);	//ws: this always confused me...
			sound (self, CHAN_VOICE, "archer/growl.wav", 1, ATTN_NORM);
...
void() archer_gibs =
{
	...
	ThrowGib ("models/archerarm.mdl", self.health);
	ThrowGib ("models/archerleg.mdl", self.health);
}
...
void monster_archer ()
{
	...
	self.th_jump = monster_jump;	self.jumpframe = $pain2;
