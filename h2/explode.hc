	...
	if(self.dmg<2.5&&self.scale>=0.1)
	{
		...
		if(self.enemy)
			if(self.enemy.health>0&&self.enemy.flags2&FL_ALIVE)//Stay with enemy;
				setorigin(self,self.enemy.origin+self.view_ofs);	//backported from PoP, was teleporting to world origin before
		self.think=BlowUp;
...
entity ignore;
	...
	else	//assasin's grenade (half the time), crusader's glyph, probably others
	{
...
//Put it right below view of player
				...
				else if(other.classname=="monster_spider"||other.model=="models/scorpion.mdl")
