...
void() FreezeTouch=
{
    ...
    if(other.takedamage&&other.health&&other.frozen<=0)//&&other.thingtype==THINGTYPE_FLESH)//ws: removed flesh requirement so it can hit buttons
	{
...
void(float altfire) FireFreeze=
{
	float wismod, intmod;
	wismod = self.wisdom;			//starts at 15-18
	intmod = self.intelligence;		//starts at 10-13
	...
	if (altfire)
	{
		...
		if (spread < 20)
			spread = 20;
		//dprint (ftos(spread)); dprint ("\n");
		
		if(self.weaponframe_cnt < ICEWAVE_SHOTS)
			self.weaponframe_cnt += 1;
		else
			self.weaponframe_cnt = ICEWAVE_SHOTS*(-1)+1;
...
void icestaff_wave (void)
{
	self.th_weapon=icestaff_wave;
	self.wfs = advanceweaponframe($power1 , $power9);
	if(self.wfs==WF_CYCLE_WRAPPED || self.bluemana < 1)
		icestaff_idle();
	else if(self.weaponframe>=$power1 && self.weaponframe<=$power8)
	{
		FireFreeze(TRUE);
	}
